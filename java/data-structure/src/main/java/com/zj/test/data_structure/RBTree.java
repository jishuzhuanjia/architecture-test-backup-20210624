package com.zj.test.data_structure;

/* @author: zhoujian
 * @qq: 2025513
 * @create-time: 2020/11/19 14:30
 * @description: 红黑树入门到自写
 * @version: 1.0
 * @finished: false
 * @finished-time:
 */
...
红黑树的算法复杂度：log2(N),10亿数据量查找只需要30次。
*/
...
public class RBTree<T extends Comparable<T>> {
    ...
    在插入新节点的时候很显然，不会违背1和3，如果插入的是根节点直接将根节点着色为黑色即可，这种情况可以忽略不计，
    所以插入节点时可能会违背了4和5，又因为插入的是红色节点因此5也不会违背，最后在插入新节点的时候我们只需要关注特性4就可以了。
    当父节点为红色的时候跟4有冲突，所以我们接下来讨论的就是这种情况。我们知道，在插入新节点之前整颗红黑树是平衡的，
    因此可以得出一个结论就是祖父节点肯定肯定是黑色的。我们现在只关注相关的节点即可，目前，我们知道了祖父的节点为黑色，
    父节点为红色，但是叔叔节点的颜色不知道，新节点的位置也不能确定，所以有2x2中情况，当叔叔节点为红色的时候，
    两种情况的处理方式是一致的，所以最后我们可以总结为3中情况：
    ...
    */
    ...
    private void remove(RBTNode<T> node) {
        ...
        if ((node.left != null) && (node.right != null)) {
            ...
            /*
            author: zj
            2020年11月20日 09:36:59

            replace(取代节点): 用来替换删除节点的节点，这里使用右子树最小值节点来替换。
             */
            ...
            if (parent == node) {
                ...
                // child不为空
                // 替代节点被移动后，需要为其子节点设置父节点为替代节点的父节点
                if (child != null)
