 ...
 * 1.构造函数 - 被动
 * 2.属性和方法 - 被动
 * 3.回调函数，接口，如Runnable接口。 - 对于线程是主动的
 * <p>
 * 线程返回结果的方式
 * 1.属性和方法,但是这个存在同步问题。
 * 2.join()。
 ...
 * <p>
 * <p>
 * public final synchronized void setName(String name) 设置线程的名字,如果调用了线程的默认无参数的
 * 构造函数,之后再调用setName修改线程的名字, 则线程将会占用之前默认构造函数分配的Thread-N名字,这就意味着这个名字不会分配给其他的线程。
 * <p>
 * <p>
 * public final void setDaemon(boolean on) 设置线程为守护线程或者用户线程
 * 这个方法首先调用isAlive,如果为true意味着线程已经开始还并没有死亡,则会抛出状态异常 当惟一运行的线程是守护进程线程时，Java虚拟机将退出。
 * 必须在启动线程之前调用此方法。
 * <p>
 * public final synchronized void join(long millis) public final synchronized
 * void join(long millis, int nanos) //第二个参数纳秒 0~999999
 * 等待/同步执行函数--注意，如果设定时间指的是等待线程的最长时间,如果指定的时间到达,线程并没有结束， 则也会停止等待,并继续执行下面的代码。
 * 等待线程死亡的时间最多为千分之一毫秒。0意味着永远等待。？？？？？？？？？？？？？？？？？？
 * 这个实现使用这个循环。基于此条件的等待调用。当线程终止此操作时。调用notifyAll方法。
 * 建议应用程序不要在线程实例上使用wait、notify或notifyAll。
 * <p>
 * <p>
 * 获取状态的函数 public long getId() //返回该线程的标识符。线程ID是创建该线程时生成的正数。线程ID是惟一的，
 * //在其生命周期内保持不变。当线程终止时，可以重用该线程ID。
 * <p>
 * public final String getName()
 * <p>
 * public final int getPriority() //获取优先级别，默认为5
 * <p>
 * public final native boolean isAlive() //测试该线程是否活动。如果线程已经启动，但尚未死亡，则该线程为活动线程。
 * <p>
 * public final ThreadGroup getThreadGroup() //获取线程组
 * <p>
 * 中断标识 public void interrupt()
 * <p>
 * 状态操作函数： start() //开始就绪 suspend() //挂起 unsafe resume() //恢复 unsafe stop() //停止
 * unsafe public static native void sleep(long millis) public static void
 * sleep(long millis, int nanos)
 * <p>
 * public static native void yield(); 给调度器的提示，即当前线程愿意放弃当前对处理器的使用。调度器可以忽略这个提示。
 */
 ...
 * <p>
 * thread.interrupt() - 打断当前状态,并将标记置true, 如果线程当前处理阻塞状态,则会打断阻塞: 会抛出
 * InterruptException并且将标记清除。
 * <p>
            ...
            *//*
            ...
            if (!isInterrupted()) {
                ...
                *//**
                 ...
                 * 首先将interrupted设置为true，并在抛出异常之前，将interrupted标记清空，因此此时catch{}语句中:
